fetch(
  "https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=30&q=%EB%8C%80%EA%B5%AC%20%EB%8F%99%EC%84%B1%EB%A1%9C&type=video&videoDefinition=high&videoEmbeddable=true&videoSyndicated=true&key=AIzaSyCQQflyO0tAoDpSR2wnLzHKGKoDgZSopRU"
)
  .then((res) => res.json())
  .then((data) => {
    const words = [
    ...
    document.querySelector("#youtube_container #Main_Grid").innerHTML = youtube
      .map(
        (tube) =>
          `<a data-video="${tube.id.videoId}"><img src="${tube.snippet.thumbnails.high.url}" /></a>`
      )
      .join("");
